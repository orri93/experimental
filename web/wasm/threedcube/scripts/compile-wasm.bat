...
SET GOS_PROJECT_WASM_SRC_DIR=%GOS_PROJECT_APP_DIR%\wasm
SET GOS_PROJECT_ASSET_DIR=%GOS_PROJECT_SRC_DIR%\assets
SET GOS_PROJECT_WASM_ASSET_DIR=%GOS_PROJECT_ASSET_DIR%\wasm

SET GOS_EMCC_OPTIONS=-Os "%GOS_PROJECT_WASM_SRC_DIR%\three-dee-cube\soil\libSOIL.bc" "%GOS_PROJECT_WASM_SRC_DIR%\three-dee-cube\3d-cube.c" -o "%GOS_PROJECT_WASM_ASSET_DIR%\3d-cube.js" -s LEGACY_GL_EMULATION=1 -Isrc/app/wasm/three-dee-cube/soil -s EXTRA_EXPORTED_RUNTIME_METHODS="['ccall']" -s FORCE_FILESYSTEM=1 -s MODULARIZE=1 -s EXPORT_NAME="Cube3dModule"
SET GOS_EMCC_CMD=emcc %GOS_EMCC_OPTIONS%

ECHO Enter %GOS_PROJECT_ROOT_DIR%
PUSHD "%GOS_PROJECT_ROOT_DIR%"
ECHO Execute emcc compiler command
ECHO %GOS_EMCC_CMD%
%GOS_EMCC_CMD%
