...
static bool active_states[GS_NUM_STATES];
static key_state_binding bindings[MAX_BINDINGS];
static unsigned int number_of_bindings;

void input_processor_init() {
  memset(active_states, 0, GS_NUM_STATES);
...
bool rm_binding(key_state_binding *binding) {
  ...
  while (b->k == binding->k && b != b_end) {
    if (b->s == binding->s && b->t == binding->t) {
      int i = (int)((b - bindings) / sizeof(key_state_binding));
      ...
      memmove(
        ...
        num_states_to_move * sizeof(key_state_binding));
      number_of_bindings -= 1;
      b_end =  bindings + number_of_bindings * sizeof(key_state_binding);
      b--;
    }
    b++;
  }
...
void process_event(SDL_Event *event, bool* goer) {
